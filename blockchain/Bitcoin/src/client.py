...
will create a client class in our code that has the ability to send and receive money"""
    ...
    def __init__(self):
        p = RSA.random_prime(n=256, k=3)  # случайное число Мерсенна порядка 2^128
        q = RSA.random_prime(n=512, k=5)  # случайное число Мерсенна порядка 2^128
        rsa = RSA(p, q)
        self.rsa_number = q * p
        self._private_key, self.public_key = rsa.generate_asymmetric_key_pair()
        self.name = Client.names[-1] + 1
        Client.names.append(self.name)
